...
#import "UtilsMacro.h"
@interface RTProjectViewController ()
...
@interface RTProjectViewController ()<RTFormViewControllerDelegate>

@end

@implementation RTProjectViewController
...
- (NSArray *)makeForm {
    // 合伙人拍照
    NSError *error = nil;
    NSArray *cellForm = [[RTLabelAndIconModel modelFromRTPartnerJsonPath:@"kPartnerJson2"] objectForKey:@"partnerForm"];
    ...
    for (NSDictionary *item in cellForm) {
        ...
        else if ([key isEqualToString:@"fields4"]){
             model = [MTLJSONAdapter modelOfClass:[RTLabelAndFieldModels4 class] fromJSONDictionary:item error:&error];
        }
...
- (UIView *)specialHeader {
    RTFormHeaderView *formImg = [[RTFormHeaderView alloc] initWithLeftCornerTitle:@"项目风采照片"];
