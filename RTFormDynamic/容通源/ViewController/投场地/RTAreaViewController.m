...
@interface RTAreaViewController ()

@end

@interface RTAreaViewController ()<RTFormViewControllerDelegate>

@end

@implementation RTAreaViewController
...
- (NSArray *)makeForm {
    // 合伙人拍照
    NSError *error = nil;
    NSArray *cellForm = [[RTLabelAndIconModel modelFromRTPartnerJsonPath:@"kPartnerJson4"] objectForKey:@"partnerForm"];
    ...
    for (NSDictionary *item in cellForm) {
        ...
        if ([key isEqualToString:@"photo"]) {
          //  model = [MTLJSONAdapter modelOfClass:[RTFormPartnerModel class] fromJSONDictionary:value error:&error];
        ...
        else if ([key isEqualToString:@"productOne"]){
            model = [MTLJSONAdapter modelOfClass:[RTFormSectionHeaderModel class] fromJSONDictionary:value error:&error];
             ...
             model = [MTLJSONAdapter modelOfClass:[RTAreaCondition class] fromJSONDictionary:value error:&error];
        }
        else if ([key isEqualToString:@"devices"]){
            model = [MTLJSONAdapter modelOfClass:[RTAreaDevicesCondition class] fromJSONDictionary:value error:&error];
        }
...
- (UIView *)specialHeader {
    RTFormHeaderView *formImg = [[RTFormHeaderView alloc] initWithLeftCornerTitle:@"场地实景图、效果图"];
